...
export async function POST(req: Request) {
  try {
    ...
    console.log("📝 Scraped text:", plainText.slice(0, 200));
    ...
    const summary = result[0]?.summary_text;
    if (!summary) throw new Error("❌ Failed to generate summary from HuggingFace");
    ...
    console.error("🔥 Caught Error in /api/summarise:", error || error);
    return NextResponse.json({ error: error|| "Failed to summarise" }, { status: 500 });
  }
}
